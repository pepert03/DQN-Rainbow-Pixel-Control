...
\newline In this project, we will focus on the \textbf{Walker2D} task \footnote{It must be noted that DPM exposes either low-dimensional state vectors or visual observations. In order to make the project as close to reality, we will work exclusively with RGB observations.}, which involves controlling a bipedal robot to walk forward as fast as possible without falling (more on this in the methodology section).
